...
export class ProjectValidationApi extends JiraCoreApi {
    ...
    async getValidKey(key: string): Promise<string> {
        return (await this.sendRequest(this.endpoint(Method.GET, '/validProjectKey', { key })));
    }

    async getValidName(name: string): Promise<string> {
        return (await this.sendRequest(this.endpoint(Method.GET, '/validProjectName', { name })));
    }
}
